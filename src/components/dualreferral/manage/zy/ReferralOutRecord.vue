<template>
  <div class="referralout-record-list">
      <div class="list-title">{{const_list.TabTitle.ReferralOut}}</div>
      <el-table
        :data="dataList"
        class="list-detail"
...
export default class ReferralOutRecord extends Vue {
  // todo: 控制权限
  @Prop() admin: string;
  ...
  getPendingAuditData() {
    ...
    Vue.prototype.$http
      .post(apiConfig.API.getReferralOutRecordInfo, requestModel)
      .then(function(response: any) {
        if (response.data.code === 0) {
