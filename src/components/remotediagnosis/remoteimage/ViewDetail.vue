...
export default class ViewDetail extends Vue {
  dialogTitle: string = "远程影像诊断";
  curMenuIndex: string = "1";
  @Prop() cursubmenuindex: number = 0;

  //初始化表单内容
  Init() {
    var path: string = "";

    this.toRecordListByPath(
      RemoteDiagnosisMenu[0].menus[0].viewmenus[this.cursubmenuindex]
    );
  }
  ...
  toRecordListByPath(obj: any) {
    if (obj.index === "2") {
      window.open(
        "http://120.26.212.233:8002/EXTERNALINTERFACE/VIEWEXI?MODE=UL&TYPE=V&LID=1&LPW=1&AN=10242358"
      );
    } else {
  ...
  //  获取菜单
  get viewDetailMenu() {
    return RemoteDiagnosisMenu[0].menus[0].viewmenus;
  }
  ...
  // 滚动条回到顶部
  scrollToTop() {
    var scrollContainer = this.$el.querySelector("#detail-content");
